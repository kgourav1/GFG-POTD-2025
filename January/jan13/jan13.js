class Solution {
    ...
    maxWater(arr) {
        let area = 0; // Variable to store the maximum water area
        let left = 0, right = arr.length - 1; // Two pointers: left and right

        // Traverse the array with two pointers
        while (left < right) {
            const h = Math.min(arr[left], arr[right]); // Calculate the height of the container
            const w = right - left;                   // Calculate the width of the container
